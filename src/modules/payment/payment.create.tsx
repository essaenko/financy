import React, {Dispatch, useState, ChangeEvent, useCallback, MouseEvent} from 'react'
...
export const PaymentCreate = (): JSX.Element => {
  ...
  const onSubmit = useCallback(async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    const result = await state.payment.createPayment(name, description, remains ?? 0);

    if (result.success) {
      history.push('/dashboard/payment')
    } else {
      setNotification('Some error occurred while creating new payment method, try again or comeback later')
    }
